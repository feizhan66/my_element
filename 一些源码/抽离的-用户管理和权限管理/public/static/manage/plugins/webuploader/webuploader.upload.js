...
;(function ($, w) {
            ...
            if (!self.options.uploadEvents.fileQueued) {
                self.BDUploader.on('fileQueued', function (file) {
                    ...
                    if (self.allowNum == 0 || self.isAllowUpload() == 0) {
                        ...
                        toastr.error('上传文件数量超出限制啦,最多上传' + self.options.fileNumLimit + '个文件', '上传信息提示:')
                        return false;
                    }
            ...
            if (!self.options.uploadEvents.uploadSuccess) {
                self.BDUploader.on('uploadSuccess', function (file, response) {
                    ...
                    if (response.status == 0) {
                        ...
                        toastr.error(response.message, '系统提示:');
                        return false;
                    }
                    self.dump('上传成功');
                    ...
                    var filebox_html = '';
                    if (type == 'image') {
                        if (self.options.fileNumLimit == 1) {
                            filebox_html = '<img src="' + response.uploadfile.url + '" class="img-responsive" />';//图片显示
                        } else {
                            filebox_html = '';//不显示图片
                        }
                    }
                        ...
                        if (self.options.fileNumLimit == 1) {
                            var plyr = '<div class="player">';
                            var plyr_img = '<video poster="" controls crossorigin>';
                            var plyr_load = '<source src="' + response.uploadfile.url + '" type="video/mp4"></video></div>';
                            filebox_html = plyr + plyr_img + plyr_load;
                        } else {
                            filebox_html = '';// 不显示视频
                        }
                        ...
                        var file = '<div class="filelists">';
                        var file_txt = '<p class="filetxt">';
                        var file_load = '文件不能预览!</p></div>';
                        filebox_html = file + file_txt + file_load;
                    }
                    else if (type == 'audio') {
                        var file = '<div class="filelists">';
                        var file_txt = '<p class="filetxt">';
                    ...
                    var filename = filename.replace(/(.*\/)*([^.]+).*/ig, "$2");

                    $fileItem.find('div.filebox').addClass(type).html(filebox_html);
                    $fileItem.find('.upload-file-info span').eq(0).attr('data-id', response.uploadfile.id)
                        .attr('data-fileurl', response.uploadfile.url)
                        .attr('data-name', response.uploadfile.name)
                        .attr('data-filename', filename)
                        .attr('data-edit', '0');
                    var responseVal = '';
                    if (self.options.hiddenValType == '1') {
                        responseVal = response.uploadfile.id;
                    } else if (self.options.hiddenValType == '2') {
                        responseVal = response.uploadfile.url + '&' + filename;
                    }
                    ...
                    if (self.allowNum == 0) {
                        $("#" + self.options.listName + "_upload").addClass("animated fadeOutLeftBig");
                    }

                    toastr.success('文件[' + file.name + ']上传成功', '系统提示:');
                });
            }
            /**
                ...
                if (name) {
                    if (isEdit == '1') {
                        if (self.options.hiddenValType == '1') {
                            ...
                            self.resetHiddenVal(name);
                        }
                        $('#' + qfid).remove();

                        if (self.allowNum > 0) {
                            $("#" + self.options.listName + "_upload").removeClass("animated fadeOutLeftBig");
                        }

                        toastr.success('文件[' + fname + ']删除成功', '系统提示:');
                    }
                    else {
                        $.post(self.options.delUrl, {
                            ...
                            if (json.status == 1) {
                                if (self.options.hiddenValType == '1') {
                                    ...
                                    self.resetHiddenVal(furl + '&' + fname);
                                }
    ...
    $.fn[pluginName].defaults = {
        ...
        swf: UPLOAD_SWF,
        server: UPLOAD_FILE_URL + '?flash=1', // 文件接收服务端。
        /**
        ...
        hiddenName: '', //文件上传隐藏域ID
        hiddenValType: '2', //文件上传隐藏域保存的值的类型   1=保存的是file表的文件编号ID，2=保存的是文件的实际路径
        listName: '', //文件上传完成显示列表区域id
        editListName: '', //文件上传完成显示列表区域id
        delUrl: DELETE_FILE_URL,
        separator: ',', //默认逗号
    }
